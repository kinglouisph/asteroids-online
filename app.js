...
setInterval(function() {
  ...
  for (var i in sockets) {
    for (var ii in asteroids) {
      ...
      if (pythag(sockets[i].x + 20 * Math.cos(rads(0 + sockets[i].angle)), sockets[i].y + 20 * Math.sin(rads(0 + sockets[i].angle)),asteroids[ii].x, asteroids[ii].y) < yeet) {sockets[i].emit('disconnect')}
      if (pythag(sockets[i].x + 20 * Math.cos(rads(0 + sockets[i].angle)), sockets[i].y + 20 * Math.sin(rads(0 + sockets[i].angle)),asteroids[ii].x, asteroids[ii].y) < yeet) {sockets[i].emit('disconnect')}
      if (pythag(sockets[i].x + 20 * Math.cos(rads(0 + sockets[i].angle)), sockets[i].y + 20 * Math.sin(rads(0 + sockets[i].angle)),asteroids[ii].x, asteroids[ii].y) < yeet) {sockets[i].emit('disconnect')}
    }
  }
